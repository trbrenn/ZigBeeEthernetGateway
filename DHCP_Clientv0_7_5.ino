...
byte remoteServer[] = {192, 168, 21, 78}; 		        //IP address of the server.
...
void processMsg() {
  ...
  if(msgType == 0x8A or msgType == 0x90 or msgType == 0x91 or msgType == 0x95 or msgType == 0x92) { //Check to see if the message type has a packet ID.
    Serial.println("    <PacketID>0</PacketID>"); 	        //We have no packet ID so output the string to the serial port for debug/testing.
