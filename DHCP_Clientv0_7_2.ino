...
void loop() {
  byte temp = 0x0; 				   	//Temp byte storage.
    ...
    serverClient = server.available();                  //Listen for connections to the local server.      						   	//Data should be prefortamed as a ZigBee message.
    if (serverClient == true) { 			//If a bit arraives at the ethernet port to the ZigBee radio.
      temp = client.read(); 			  	//Read the byte and store it in the c varible.
      if(temp ==0x7E) {                                 //If this byte is a Frame Delimiter start to process it
        ...
        while(serverClient) {                           //read everything from the client connect to the local server.
          message[length++] = client.read();            //store the bytes in the message buffer.                 
        } 
        int index = 0;
        switch (errorCheckMsg()) {                      //Check to make sure the message is in a valid format
          case 0:                                       //The message is valid 
                                          //Create and zero out an index.
            ...
            server.println("OK");                       //Let the client know the packet was sent and it can disconnect now.
            break;
          case 1:                                                //This packet was not a valid type.
            ...
            server.println("Error: Invalid Message Type");       //Let the client know the packet was malformed and it can disconnect now. 
            break;
          case 2:                                                //This packet was not the correct length.
            ...
            server.println("Error: Incorrect Message Length");   //Let the client know the packet was malformed and it can disconnect now.
            break;
          case 3:                                                //The offset for this packet is wrong.
            ...
            server.println("Error: Incorrect Message Offset");   //Let the client know the packet has the wrong offset and it can disconnect now.
            break;
          Default:                                               //This just failed.
            ...
            server.println("Something Really Messed Up!");       //Let the client know the packet has failed and it can disconnect now.
            break;
        }
      }
      else {
        Serial.println("No Start Delimiter");                    //Let the Serial monitor know the packet has no Start Delimiter.
        server.println("Error: No Start Delimiter");             //Let the client know the packet has no Start Delimiter.
        while(serverClient) {                                    //Flush out the trash.
          message[length++] = client.read();
        } 
      }  
    }
}
...
void processMsg() {
  ...
  delay(25); 				   	//Delays to make sure we catch each bit.
  length = xbee.read(); 			   	//Get the length of the message.
  Serial.print("    <Length>\n      "); 	   			//output the byte to the serial port for debug/testing.
  client.print("<Length>"); 	   			//output the byte to the ethernet shield and send to the server.
  Serial.println(length, HEX); 	   			//output the byte to the serial port for debug/testing.
  client.print(length, HEX); 	   			//output the byte to the ethernet shield and send to the server.
  ...
  length--;				        //Account the mesType byte removed from the stream
  Serial.print("    <MsgType>\n      "); 	   			//output the char to the serial port for debug/testing.
  client.print("<MsgType>"); 	   			//output the char to the ethernet shield and send to the server.
  Serial.println(msgType, HEX); 	   			//output the byte to the serial port for debug/testing.
  ...
  packetID = xbee.read(); 			//Get the Packet ID of the message we're recieving.
  length--;				        //Account the mesType byte removed from the stream.
  Serial.print("    <PacketID>\n      "); 	   			//output the char to the serial port for debug/testing.
  client.print("<PacketID>"); 	   			//output the char to the ethernet shield and send to the server.
  Serial.println(packetID, HEX); 	   			//output the byte to the serial port for debug/testing.
  client.print(packetID, HEX); 	   			//output the byte to the ethernet shield and send to the server.
  ...
  Serial.println("\n    </MsgData>"); 	   			//output the char to the serial port for debug/testing.
  client.print("</MsgData>"); 	   			//output the char to the ethernet shield and send to the server.
  ...
  Serial.println(offset, HEX); 	   			//output the byte to the serial port for debug/testing.
